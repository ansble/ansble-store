    ...
    var action = {
        ...
        , viewMe : function(objectIn){
            ...
            newView.listen('state:change', function(stateID){
                ...
                if(that.stateEvent.match(/[\*]$/)){
                    routeMagic = that.stateEvent.replace(/[\*]$/, '') + '.+';
                    if(stateID.match(routeMagic) !== null){
                        //remove the old data event
                        that.silence({eventName: 'data:set:' + that.dataID, scope: that});

                        //set the new data id
                        that.dataID = stateID.replace(that.stateEvent.replace(/[\*]$/, ''), '');

                        //set the new data event
                        that.requiredEvent('data:set:' + that.dataID, function(dataIn){
                            this.viewData = dataIn;
                        }, that, true);
                    }
                }
            ...
            newView.listen('navigate:to', function(stateID){
                var that = this
                    , stateID = stateID;
                       ...
                       // var location = this.attributes.href.textContent;
                        ...
                        if(elem.tagName.toLowerCase() === 'a'){
                            ...
                            if(location.match(/http:/)){
                                ...
                                events.emit('navigate:to', location);
                                e.preventDefault();
                            }
                        }
                    });
